...
class HabitManager:
    ...
    def add_habit(self, name: str, description: str,periodicity): #add new habit and stores it
        ...
        self.habits.append(Habit(name, description, periodicity))
        self.save()
    ...
    def complete_habit(self, name: str) -> bool: #marks a habit at complete
        habit = self.find_habit(name)

        if habit is None:
            return False

        habit.mark_complete()  # or mark_done() if you keep the original name
        self.save()
        return True
